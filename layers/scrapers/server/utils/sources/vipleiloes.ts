...
async function collectFromCurrentPageHtml(page: Page, all: RawScrapedVehicle[], seenUrls: Set<string>, classification: VipClassification, log: (msg: string) => void): Promise<{ added: number; parsed: SearchFragmentParseResult }> {
  ...
  for (const vehicle of parsed.vehicles) {
    ...
    seenUrls.add(vehicle.url); all.push(vehicle); added += 1
  }
  return { added, parsed }
}
  ...
  try {
    ...
    for (const classification of DEFAULT_CLASSIFICATIONS) {
      ...
      for (const candidate of startCandidates) {
        ...
        if (protection) { log('[vipleiloes] Bloqueio anti-bot persistente.'); return [] }
        const html = await page.content().catch(() => '')
      ...
      while (pageAttempt < maxPages) {
        if (!ajaxUrl) {
          ...
          await sleep(REQUEST_DELAY_MS); continue
        }

        const normalizedAjaxUrl = ensureClassificationQuery(ajaxUrl.replace(/&amp;/g, '&'), classification)
        if (visitedAjaxUrls.has(normalizedAjaxUrl)) { log(`[vipleiloes] Loop de paginação detectado. Encerrando.`); break }
        ...
        if (!partial.ok) {
          ...
          if (!ajaxUrl) { const clicked = await clickLoadMoreIfAvailable(page); if (!clicked) break; const domC = await collectFromCurrentPageHtml(page, all, seenUrls, classification, log); ajaxUrl = domC.parsed.nextAjaxUrl ? ensureClassificationQuery(domC.parsed.nextAjaxUrl, classification) : null }
          await sleep(REQUEST_DELAY_MS); continue
        }
      ...
      if (pageAttempt >= maxPages && ajaxUrl) log(`[vipleiloes][${classification.name}] Limite de ${maxPages} página(s) atingido.`)
