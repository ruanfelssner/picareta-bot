...
function extractPartSignals(normalized: string): number {
...
function lineMentionsAnyModel(line: string, modelTokens: string[]): boolean {
  return modelTokens.some((token) => line.includes(token));
}

function lineIncludesYear(line: string, year: number): boolean {
  if (new RegExp(`\\b${year}\\b`).test(line)) return true;
  ...
  if (rangeMatch) {
    ...
    const end = Number.parseInt(
      rangeMatch[0].match(/(?:a|até|-)\s*((19|20)\d{2})/i)?.[1] || "",
      10
    );
...
function hasYearModelCoherenceIssue(text: string, intent: AutoPartsIntent): boolean {
  if (!intent.yearToken || intent.modelTokens.length === 0) {
    return false;
  }

  const targetYear = Number.parseInt(intent.yearToken, 10);
  if (Number.isNaN(targetYear)) return false;
...
function assessAutoParts(
  text: string,
  matchedTokensCount: number,
  requiredTokensCount: number,
  intent: AutoPartsIntent
): SemanticAssessment {
...
export const autoPartsRuleFactory: SemanticRuleFactory = {
  name: "auto_parts",
  ...
  create: (tokens): SemanticRuntime => {
    ...
    return {
      ...
      assess: (text, matchedTokensCount) =>
        assessAutoParts(text, matchedTokensCount, requiredTokens.length, intent)
    };
  }
};
