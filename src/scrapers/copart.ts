...
function getLocations(filters: AuctionFilters): string[] {
  ...
  const isOnlyLegacyDefaultLocations =
    normalizedFromFilters.length === normalizedDefaults.size &&
...
function collectLotRecords(obj: unknown, collector: CopartLot[], depth = 0): void {
  ...
  if (Array.isArray(obj)) {
    for (const item of obj) collectLotRecords(item, collector, depth + 1);
    return;
  }
...
async function detectCopartProtectionWithRetry(
  page: Page,
  log: (msg: string) => void,
  contextLabel: string
): Promise<string | null> {
...
async function fetchLotsFromCopartApi(
  page: Page,
  miscFilter: string,
  log: (msg: string) => void
): Promise<CopartLot[]> {
  const callApiPage = async (pageNumber: number, draw: number) => {
    const body = buildCopartSearchPostBody(
      miscFilter,
      DEFAULT_COPART_CATEGORIES,
      { page: pageNumber, size: COPART_PAGE_SIZE, draw }
    );
    return page.evaluate(
      async ({ url, payload }) => {
        ...
        let json: CopartApiResult | null = null;
        try {
          json = JSON.parse(text) as CopartApiResult;
        } catch {
          // ignore
        }
        return {
...
function findAuctionIdsForLocations(obj: unknown, locations: string[], depth = 0): number[] {
  ...
  if (Array.isArray(obj)) {
    for (const item of obj) ids.push(...findAuctionIdsForLocations(item, locations, depth + 1));
    return ids;
  }
...
export async function scrapeCopart(
  ...
  options?: { headless?: boolean; log?: (msg: string) => void; profilePath?: string }
  ...
  try {
    ...
    for (const target of saleTargets.slice(0, 3)) {
      ...
      log(
        `[copart] intercepted JSONs: ${intercepted.length} | ` +
      ...
      for (const lot of lots) {
        ...
        const damageParts = [damageRaw.trim(), damageClassificationRaw.trim()]
          ...
          .filter((part, idx, arr) =>
            arr.findIndex((candidate) => normalizeToken(candidate) === normalizeToken(part)) === idx
          );
      ...
      if (skippedLargeDamageByTarget > 0) {
        log(
          `[copart] ${targetLabel}: ${skippedLargeDamageByTarget} lote(s) ignorado(s) por grande monta/sucata.`
        );
      }
    }
  } finally {
