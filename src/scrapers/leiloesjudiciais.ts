...
function parseCards(html: string, log: (msg: string) => void): AuctionVehicle[] {
  ...
  $("a.card-lote-leilao").each((_i, el) => {
    ...
    const finalPriceValue =
      currentBidValue != null && currentBidValue > 0
        ? currentBidValue
...
async function fetchHtml(url: string, log: (msg: string) => void): Promise<string | null> {
  try {
    ...
    log(
      `[leiloesjudiciais] Erro em ${url}: ${error instanceof Error ? error.message : String(error)}`
    );
    return null;
  }
}
...
export async function scrapeLeiloesJudiciais(
  ...
  log(
    `[leiloesjudiciais] Página(s): total=${discoveredPages} | limite=${maxPages} | varrendo=${totalPages}.`
  );
