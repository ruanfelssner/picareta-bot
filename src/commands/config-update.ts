import type { AuctionFilters, MongoConfig } from "../integrations/mongo.js";
...
type ParsedConfig =
  | { action: "show"; value: null }
  | { action: "set-locations"; value: string }
  | { action: "set-states"; value: string }
  | { action: "set-cities"; value: string };

function parseConfigArg(arg: string): ParsedConfig | null {
  const raw = arg.trim();
...
function formatComboRule(rule: AuctionFilters["comboRules"][number], index: number): string {
  const chunks = [
...
function formatFiltersMessage(filters: AuctionFilters): string {
...
function parseListOrClearInput(value: string): {
  clear: boolean;
  values: string[];
} {
  ...
  if (parsed.action === "set-locations") {
    ...
    if (!clear && values.length === 0) {
      return {
        ok: false,
        message:
          "❌ Informe ao menos um local (ex: /filtro locais Curitiba - PR, Canoas - RS) " +
