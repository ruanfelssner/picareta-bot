...
import {
  extractLocationStateUf,
  evaluateTermMatch,
  extractLocation,
  extractPrice,
  extractTitle,
  isGenericMarketplaceTitle,
  isLikelyLoginScreen,
  looksLikeBlockingVerification,
  normalizeMarketplaceItemUrl,
  normalizeText,
  randomBetween,
  stripMarketplaceSuggestionSections,
  sleep,
  type TermMatchInput
} from "./utils.js";
import type { RelevanceLevel, SemanticRuntime } from "./rules/index.js";
...
export type MarketplaceResult = {
  ...
  cardMatchedTokens: string[];
  detailMatchedTokens: string[];
...
type ScrapeMarketplaceParams = {
  searchTerm: string;
  maxScrolls: number;
  headless: boolean;
  profilePath: string;
  requireTermMatch: boolean;
  enrichItemDetails: boolean;
  maxDetailPages: number;
  minMatchedTokensForDetailOpen: number;
  maxFallbackResults: number;
  searchConfig: MarketplaceSearchConfig;
  matchConfig: TermMatchConfig;
...
function buildMatchInput(matchConfig: TermMatchConfig): TermMatchInput {
  return {
...
function applyMatchAndSemantic(
  ...
  const hasMissingPrice =
    !result.priceRaw || result.priceRaw.trim() === "" || result.priceRaw.trim() === "-";
...
async function hasLoginInputs(page: Page): Promise<boolean> {
...
async function detectManualIntervention(page: Page): Promise<boolean> {
...
async function detectBlockingVerification(page: Page): Promise<string | null> {
...
async function countMarketplaceCards(page: Page): Promise<number> {
  return page.locator(MARKETPLACE_CARD_SELECTOR).count();
}

async function readCardSnapshot(page: Page): Promise<{ count: number; lastHref: string | null }> {
...
async function advanceMarketplaceFeed(page: Page): Promise<ScrollAttemptResult> {
  const before = await readCardSnapshot(page);
  const links = page.locator(MARKETPLACE_CARD_SELECTOR);

  const executeStep = async (strategy: string, action: () => Promise<void>): Promise<ScrollAttemptResult | null> => {
    await action();
...
async function collectVisibleCards(page: Page): Promise<RawCard[]> {
  const links = page.locator(MARKETPLACE_CARD_SELECTOR);
  const count = await links.count();
  const items: RawCard[] = [];
...
async function openMarketplaceHome(page: Page): Promise<void> {
  try {
...
function buildSearchUrl(searchTerm: string, searchConfig: MarketplaceSearchConfig): string {
...
async function readItemVisibleText(context: BrowserContext, itemUrl: string): Promise<string> {
  ...
  try {
    ...
    if (blockReason) {
      throw new Error(
        `Processo interrompido por tela de verificação/bloqueio ao abrir anúncio. Motivo: ${blockReason}`
      );
    }
...
async function enrichResultsWithItemDetails(
  ...
  if (limit <= 0) {
    console.log(
      `Nenhum anúncio com match mínimo (${minMatchedTokensForDetailOpen}) para abrir detalhe.`
    );
    return;
  }
  ...
  for (let index = 0; index < limit; index += 1) {
    ...
    if (
      (!result.titleRaw || /^R\$/.test(result.titleRaw) || isGenericMarketplaceTitle(result.titleRaw)) &&
      betterTitle &&
...
export async function runMarketplaceSearch({
  ...
  try {
    ...
    if (preSearchBlockReason) {
      throw new Error(
        `Processo interrompido por tela de verificação/bloqueio. Motivo: ${preSearchBlockReason}`
      );
    }
    ...
    if (postSearchBlockReason) {
      throw new Error(
        `Processo interrompido por tela de verificação/bloqueio. Motivo: ${postSearchBlockReason}`
      );
    }
    ...
    for (let scrollIndex = 1; scrollIndex <= maxScrolls; scrollIndex += 1) {
      ...
      console.log(
        `Coleta ${scrollIndex}/${maxScrolls} concluída: +${addedInThisStep} novo(s), ${unique.size} anúncio(s) únicos.`
      );
      ...
      if (scrollIndex < maxScrolls) {
        ...
        if (scrollAttempt.moved) {
          staleScrolls = 0;
          console.log(
            `Scroll ${scrollIndex}/${maxScrolls}: avanço por ${scrollAttempt.strategy} ` +
              `(cards ${scrollAttempt.beforeCount} -> ${scrollAttempt.afterCount}).`
          );
        } else {
          staleScrolls += 1;
          console.log(
            `Scroll ${scrollIndex}/${maxScrolls}: sem avanço detectado ` +
              `(tentativa ${staleScrolls}/2, strategy=${scrollAttempt.strategy}).`
          );
          if (staleScrolls >= 2) {
            console.log(
              "Interrompendo coleta antes do limite: feed sem progresso após múltiplas tentativas de scroll."
            );
            break;
          }
        }
      ...
      if (scrollBlockReason) {
        throw new Error(
          `Processo interrompido por tela de verificação/bloqueio durante o scroll. Motivo: ${scrollBlockReason}`
        );
      }
    }
    ...
    if (requireTermMatch && filtered.length === 0 && results.length > 0) {
      ...
      if (relaxed.length > 0) {
        const limited =
          maxFallbackResults > 0 ? relaxed.slice(0, maxFallbackResults) : relaxed;
        console.log(
          `Filtro estrito retornou 0. Aplicando fallback para anúncios com pelo menos 1 token casado (${limited.length}/${relaxed.length}).`
        );
        filtered = limited;
      }
    }
