...
import { isResultSlightlyDistant, parseBoolean, parsePositiveInt } from "./utils.js";
dotenv.config();
...
function printTopResults(results, limit = 20) {
...
async function main() {
    ...
    const { results, collectedCandidates, semanticRuleName, effectiveSearchTerm, conditionMode } = await executeSearchRun({
        searchTerm,
        maxScrolls,
        headless,
        profilePath,
    ...
    const discardedByRelevance = collectedCandidates.filter((item) => item.relevanceLevel === "descartar").length;
    const rejectedByMatch = collectedCandidates.filter((item) => item.relevanceLevel !== "descartar" && !item.matchApproved).length;
