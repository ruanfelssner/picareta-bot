...
import { claimNextAnyPendingCommand, getMongoDataConfigFromEnv, getMongoQueueConfigFromEnv, isMarketplaceCommandCancelRequested, markMarketplaceCommandCancelled, markMarketplaceCommandDone, markMarketplaceCommandFailed, touchMarketplaceWorkerHeartbeat } from "./integrations/mongo.js";
...
import { parseCronSchedule, parseAuctionGroupPhone, startDailyAuctionScheduler } from "./scheduler/daily-auction.js";
dotenv.config();
...
function buildPrefixedLogger(prefix) {
    return (message) => {
        console.log(`${prefix} ${message}`);
    };
}
function createCancelChecker(mongoUriEnabled, commandId) {
    let lastCheckAt = 0;
    let cached = false;
    return async (mongoConfig) => {
...
async function notifyGroupText(zApiConfig, groupPhone, message, logger) {
...
async function processSearchCommand(command, options, logger) {
    if (!command.commandArg) {
        await markMarketplaceCommandFailed(options.queueMongoConfig, command.id, "Comando SEARCH sem argumento.", { reason: "missing_command_arg" });
        logger("Finalizado com falha: comando sem argumento.");
        return;
    }
    await notifyGroupText(options.zApiConfig, command.groupPhone, `🔎 Busca iniciada por "${command.commandArg}".`, logger);
    ...
    if (await shouldCancel()) {
        ...
        await notifyGroupText(options.zApiConfig, command.groupPhone, `⏹️ Busca cancelada para "${command.commandArg}".`, logger);
        return;
    }
    ...
    await notifyGroupText(options.zApiConfig, command.groupPhone, `✅ Busca finalizada para "${run.effectiveSearchTerm}". ${run.results.length} anúncio(s) encontrados.`, logger);
}
async function processAuctionSearchCommand(command, options, logger) {
    const startedAt = Date.now();
...
async function processFipeLookupCommand(command, options, logger) {
    if (!command.commandArg) {
...
async function processContactSearchCommand(command, options, logger) {
...
async function processContactInsertCommand(command, options, logger) {
    // senderPhone vem em metadata se disponível
    const senderPhone = command.senderPhone ?? command.groupPhone;
    const result = await handleContactInsert(command.commandArg, senderPhone, options.dataMongoConfig);
...
async function processConfigUpdateCommand(command, options, logger) {
...
async function processOneCommand(command, options) {
    if (!command) {
        return;
    }
    ...
    try {
        switch (command.commandType) {
            ...
            default:
                ...
                await markMarketplaceCommandFailed(options.queueMongoConfig, command.id, `Tipo de comando não suportado: ${command.commandType}`);
        }
    }
    catch (error) {
        ...
        if (isCancelled) {
            ...
            await notifyGroupText(options.zApiConfig, command.groupPhone, `⏹️ Operação cancelada.`, logger);
        }
        else {
            ...
            await notifyGroupText(options.zApiConfig, command.groupPhone, `❌ Operação falhou: ${message.slice(0, 100)}`, logger);
        }
    }
    finally {
        await options.setHeartbeatState({
...
async function main() {
    ...
    const heartbeatSeconds = parsePositiveInt(process.env.WORKER_HEARTBEAT_SECONDS, DEFAULT_HEARTBEAT_SECONDS);
    ...
    if (!queueMongoConfig.enabled) {
        throw new Error("Mongo da fila não configurado. Defina MONGO_QUEUE_URI (ou MONGO_URI) para o worker consumir comandos.");
    }
    ...
    try {
        while (keepRunning) {
            try {
                ...
                const message = error instanceof Error ? error.message : String(error);
                const isMongoConnectivityError = /ETIMEDOUT|ECONNRESET|ECONNREFUSED|Mongo(Network|ServerSelection)|server selection/i.test(message);
                if (isMongoConnectivityError) {
                    console.error(`Mongo indisponível temporariamente (${message}). Tentando novamente em ${Math.round(MONGO_ERROR_BACKOFF_MS / 1000)}s...`);
