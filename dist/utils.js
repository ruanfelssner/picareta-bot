...
function isFuzzyComparableToken(token) {
    return token.length >= 5 && /^[a-z]+$/i.test(token);
}
function hasMaxDistanceOne(a, b) {
    if (a === b)
        return true;
    const la = a.length;
    const lb = b.length;
...
function hasFuzzyTokenMatch(token, textTokens) {
    if (!isFuzzyComparableToken(token)) {
        return false;
    }
    for (const candidate of textTokens) {
        if (!isFuzzyComparableToken(candidate))
            continue;
...
export function looksLikeLocationLine(line) {
    ...
    const hasKnownCity = CITY_HINTS.some((city) => normalized.includes(normalizeForMatch(city)));
    if (hasKnownCity) {
    ...
    const isCityUfPattern = /[A-Za-zÀ-ÿ'\-\s]{3,}\s*[-,]\s*(PR|SC|SP|RS)\b/i.test(trimmed) ||
...
export function parsePriceToCents(priceRaw) {
    if (!priceRaw) {
        return null;
    }
...
export function formatIsoToPtBr(isoDate) {
...
export function isGenericMarketplaceTitle(title) {
    if (!title) {
        return true;
    }
...
export function tokenizeSearchTerm(searchTerm) {
...
export function parseCsvTerms(value) {
    if (!value || value.trim() === "") {
        return [];
    }
...
export function evaluateTermMatch(text, input) {
    ...
    const matchedTokens = requiredTokens.filter((token) => tokensInText.has(token) || hasFuzzyTokenMatch(token, tokenCandidates));
...
export function findMatchedTokens(text, tokens) {
...
export function shouldKeepByTermMatch(searchTerm, matchedTokens) {
...
export function normalizeMarketplaceItemUrl(url) {
    try {
...
export function looksLikeBlockingVerification(text) {
...
export function isLocationInStates(locationRaw, states) {
...
export function isLocationOutsideParana(locationRaw) {
    const uf = extractLocationStateUf(locationRaw);
    if (!uf) {
        return false;
    }
...
export function extractListingAge(rawText) {
...
export function stripMarketplaceSuggestionSections(text) {
    const normalized = normalizeText(text);
    if (!normalized) {
        return normalized;
    }
    ...
    const markerIndex = lines.findIndex((line) => markerPatterns.some((pattern) => pattern.test(line)));
    if (markerIndex <= 0) {
        return normalized;
    }
