import { runMarketplaceSearch } from "./facebook-marketplace.js";
import { loadLatestSentStateByDestination, persistRunToMongo, persistZApiDispatchesToMongo } from "./integrations/mongo.js";
...
function parseConditionDirective(searchTerm) {
    ...
    for (let index = 0; index < rawTokens.length; index += 1) {
        ...
        const isSemiNovoPair = normalized === "semi" &&
...
function resolveZApiConfig(baseConfig, destinationOverride) {
    const destination = destinationOverride?.trim() || baseConfig.phone;
    const hasCredentials = Boolean(baseConfig.instanceId && baseConfig.token && baseConfig.clientToken && destination);
    return {
        ...baseConfig,
        phone: destination,
...
export async function executeSearchRun(input) {
    ...
    const searchConfig = conditionDirective.itemConditionOverride
        ? {
    ...
    if (zApiConfig.enabled) {
        try {
            ...
            logger(`Notificação em fila: ativa (histórico carregado de ${historyByUrl.size} anúncio(s) já enviados).`);
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            logger(`Notificação em fila: falha ao carregar histórico (${message}). Continuando sem histórico anterior.`);
    ...
    const results = await runMarketplaceSearch({
        ...
        onUniqueResult: (item) => {
            if (!liveNotifier)
                return;
            if (item.relevanceLevel === "descartar" || !item.matchApproved)
                return;
    ...
    if (input.mongoConfig.enabled) {
        try {
            ...
            logger(`Mongo: execução salva (runId=${persistResult.runId ?? "-"}) e ${persistResult.upsertedListings} anúncio(s) atualizados/inseridos.`);
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
    ...
    if (zApiConfig.enabled && liveNotifier) {
        try {
            ...
            logger(`Z-API fila: enfileirados ${dispatchStats.queued}, enviados ${dispatchStats.sent}, pulados ${dispatchStats.skipped}, falhas ${dispatchStats.failed}, ignorados ${dispatchStats.ignored}.`);
