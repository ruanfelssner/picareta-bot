...
function formatPriceRawFromNumber(value) {
    if (!Number.isFinite(Number(value)) || Number(value) <= 0)
        return null;
    return `R$ ${Math.round(Number(value)).toLocaleString("pt-BR")}`;
}
function extractFipeFromTemplate(offer) {
...
function extractFipeFromTexts(texts) {
    const patterns = [
...
function normalizeSlugPiece(raw) {
    return raw
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
        .toLowerCase()
...
function parseBrandAndModelFromTitle(rawTitle) {
    const cleaned = rawTitle
        .replace(/\s+/g, " ")
        .replace(/^(?:SUCATA\s+DE\s+)?(?:DIREITOS?\s+SOBRE\s+)?(?:CARRO|VE[IÍ]CULO|CAMINHONETE|MOTO|MOTOCICLETA|CAMINH[AÃ]O)\s+/i, "")
        .trim();
...
function extractOfferUrlsById(html) {
    const out = new Map();
...
function buildFallbackOfferUrl(title, id) {
...
function parseOffer(offer, pageUrlMap) {
    ...
    const title = (product?.shortDesc ?? "").trim() ||
    ...
    const detailPriceRaw = offerDetail?.currentMinBidFormatted ??
        offerDetail?.initialBidValueFormatted ??
        offer.priceFormatted ??
        null;
    const detailPrice = (typeof offerDetail?.currentMinBid === "number" && offerDetail.currentMinBid > 0
    ...
    const finalPriceRaw = parsedPriceFromRaw.priceRaw ??
...
export async function scrapeSuperbid(_filters, options) {
    ...
    const pageSize = typeof firstList.limit === "number" && firstList.limit > 0
    ...
    log(`[superbid] Página(s): total=${discoveredPages} | limite=${maxPages} | varrendo=${totalPages} ` +
        `(total estimado=${total}, pageSize=${pageSize}).`);
    const appendOffers = (offers, html) => {
