...
function canonicalizeWheelTokens(tokens) {
...
function buildCardDescriptorTokens(tokens) {
    return tokens.filter((token) => {
        if (token === "roda" || token === "rodas")
            return false;
        if (/^[4-8]x\d{2,3}$/.test(token))
            return false;
        if (/^aro(1[3-9]|2[0-2])$/.test(token))
            return false;
...
function parseWheelIntent(tokens) {
    let wheelSize = null;
    let boltPattern = null;
    for (const token of tokens) {
...
function buildWheelExcludes(tokens, intent) {
...
function extractSizeSignals(normalized) {
...
function extractBoltSignals(normalized) {
    const values = new Set();
...
function extractBoltHoleSignals(normalized) {
    const values = new Set();
...
function assessWheels(text, matchedTokensCount, intent, hasExcludedTokens) {
    ...
    const hasConflictingSize = intent.wheelSize != null && hasSizeInfo && sizeValues.some((value) => value !== intent.wheelSize);
    const hasConflictingBolt = intent.boltPattern != null &&
        hasBoltInfo &&
        boltValues.some((value) => value !== intent.boltPattern);
    const hasConflictingBoltHoles = targetHoleCount != null &&
        hasBoltHoleInfo &&
...
export const wheelsRuleFactory = {
    name: "wheels",
    supports: (tokens) => {
